...
/usr/bin/gfwlist2dnsmasq.sh -d 127.0.0.1 -p 5300 -o $TEMPFILE if [ $? -eq 0 ]; then
...
wget 'http://ftp.apnic.net/apnic/stats/apnic/delegated-apnic-latest' -q -O $TEMPFILE if [ $? -eq 0 ]; then
    cat $TEMPFILE|awk -F\| '/CN\|ipv4/ { printf("%s/%d\n", $4,
32-log($5)/log(2)) }' > $CHNROUTE_TEMPFILE
